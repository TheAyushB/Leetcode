...
class Solution {
    public int coinChange(int[] coins, int amount) {
        ...
        for(int i = 0; i <= n; i ++)
            ...
            {
                if(remAmount == 0)
                minCoinsDp[remAmount] = 0;
                else if(i == 0)
                    minCoinsDp[remAmount] = amount + 1; // invalid
                else
                {
                    int dTake = minCoinsDp[remAmount];
                    int take = Integer.MAX_VALUE;
                    if(remAmount >= coins[i - 1])
                        take = 1 + minCoinsDp[remAmount - coins[i - 1]];
                    minCoinsDp[remAmount] = Math.min(take, dTake);
                } 
            }
        
        if(minCoinsDp[amount] > amount)
            minCoinsDp[amount] = -1;
